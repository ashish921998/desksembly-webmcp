...
- [ ] **10. Finish exact review, real cart transition, and checkout handoff**
  Spec ref: `spec.md > Cart Review Gate`, `Most Important Data Flow > Review to real cart`, `prd.md > Epic 6`, `Epic 7`
  What to build: Complete ReviewPanel, exact line/variant/price/market warnings, approval invalidation on any scene/price change, one-time cart gate, authoritative cart event/subscription reconciliation, partial-failure UI, cart drawer bridge, post-confirmation cart animation, and checkout enablement/handoff. Approval remains the single required human click.
  Acceptance: The review matches current stable scene; stale/changed review cannot approve; unapproved native cart calls fail; approved lines update the real Shopify cart; only accepted variants animate; mismatches and partial failures block checkout; exact reconciliation enables a normal Shopify checkout URL; no payment is autonomous.
  Verify: Run `npm run test:e2e -- review-cart`; manually test price change, partial failure, repeated approval, pre-existing unrelated cart line, and exact checkout handoff; compare review and cart snapshots; run `npm run build`; commit as `feat: reconcile reviewed desk with shopify cart`.

- [ ] **11. Harden, evaluate, deploy, and record the submission build**
